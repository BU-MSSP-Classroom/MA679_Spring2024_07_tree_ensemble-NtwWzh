...
set.seed(679)
sample_data <- sample.split(Boston$medv,SplitRatio = 0.8)
train_boston <- subset(Boston, select=-c(medv), sample_data==TRUE)
test_boston <- subset(Boston,select=-c(medv),sample_data==FALSE)
train_boston_y <- subset(Boston$medv,sample_data==TRUE)
test_boston_y <- subset(Boston$medv,sample_data==FALSE)

p=13
boston_model <- randomForest(train_boston_y~., data=train_boston,mtry=12,ntree=25)
boston_model

yhat_boston <- predict(boston_model,newdata=test_boston)
plot(yhat_boston,test_boston_y)
abline(0,1)
mean((yhat_boston-test_boston_y)^2)

rf_boston <- randomForest(train_boston_y~.,data=train_boston,mtry=12,importance=TRUE)
yhat_rf <- predict(rf_boston,newdata=test_boston)
importance(rf_boston)
varImpPlot(rf_boston)
```

It looks like the MSE is getting larger
...
```{r}
sample_data2 <- sample.split(Carseats, SplitRatio = 0.8)
train_car <- subset(Carseats,sample_data2==TRUE)
test_car <- subset(Carseats,sample_data2==FALSE)
```

(b) Fit a regression tree to the training set. Plot the tree, and interpret the results. What test MSE do you obtain?
```{r}
set.seed(679)
car_tree <- tree(Sales~.,data= train_car)
summary(car_tree)
plot(car_tree)
text(car_tree,pretty=0,cex=0.6)

prediction <- predict(car_tree,test_car)
car_mse <- mean((prediction-test_car$Sales)^2)
car_mse
```
...
prune_car <- prune.tree(car_tree,best=5)
plot(prune_car)
text(prune_car, pretty=0)

car_yhat <- predict(prune_car,newdata = test_car)
plot(car_yhat,test_car$Sales)
abline(0,1)
cv_car_mse <- mean((car_yhat-test_car$Sales)^2)
cv_car_mse
```
...
```{r}
set.seed(679)
bag_car <- randomForest(Sales~.,train_car,mtry=8,importance=TRUE)
bag_car

yhat_bag <- predict(bag_car,newdata = test_car)
plot(yhat_bag,test_car$Sales)
abline(0,1)
bag_mean <- mean((yhat_bag-test_car$Sales)^2)
bag_mean
```
...
```{r}
set.seed(679)
rd_car1 <- randomForest(Sales~.,train_car,mtry=10/2,importance=TRUE)
rd_car2 <- randomForest(Sales~.,train_car,mtry=10/4,importance=TRUE)
rd_car3 <- randomForest(Sales~.,train_car,mtry=10/6,importance=TRUE)

importance(rd_car1)
importance(rd_car2)
importance(rd_car3)

varImpPlot(rd_car1,cex=0.7)
varImpPlot(rd_car2,cex=0.7)
varImpPlot(rd_car3,cex=0.7)

rd1_mse <- mean((predict(rd_car1,newdata = test_car)-test_car$Sales)^2)
rd2_mse <- mean((predict(rd_car2,newdata = test_car)-test_car$Sales)^2)
rd3_mse <- mean((predict(rd_car3,newdata = test_car)-test_car$Sales)^2)

c(rd1_mse,rd2_mse,rd3_mse)
```
(f) Now analyze the data using BART, and report your results.
```{r}
set.seed(679)
column_names <- names(Carseats)[2:9]
column_names <- column_names[-(7-1)]
x <- Carseats[, column_names]
y <- Carseats[,"Sales"]
xtrain <- x[train_car[[1]], ]
ytrain <- y[train_car$Sales]
xtest <- x[-train_car[[1]],]
ytest <- y[-train_car$Sales]
barfit <- gbart(xtrain,ytrain,x.test=xtest)
bart_yhat <- barfit$yhat.test.mean
bart_mean <- mean((ytest-bart_yhat)^2)
bart_mean
ord <- order(barfit$varcount.mean,decreasing=TRUE)
...
Your code:
```{r,echo=TRUE}
oj_tree <- tree(Purchase~.,train_oj,)
summary(oj_tree)
```
...
~~~
the training error rate is 0.1512, there are 8 terminal nodes in the tree
...
~~~
Brach 4 has a terminal node, the split is LoycalCH < 0.276142, it has 174 observations, with each observation belonging to MM. Therefore, the final prediction for this branch is MM.
...
Your code:
```{r,echo=TRUE}
oj_yhat <- predict(oj_tree,newdata=test_oj,type='class')
table(oj_yhat,test_oj$Purchase)
```

Your answer:

~~~
The test error rate is 19+37/147+19+37+67=0.2074.
...
~~~
at size8, the cv classification error rate is the lowest
...
pred_prune_oj <- predict(prune_oj, newdata = train_oj, type = "class")
table(pred_prune_oj,train_oj$Purchase)
```

Your answer:

~~~
the error rate is 76+57/76+57+411+256=0.16625
...
```{r,echo=TRUE}
unprune_oj_train <- predict(oj_tree,newdata=train_oj,type='class')
table(unprune_oj_train,train_oj$Purchase)
```

Your answer:

~~~
the train error rate is 30+91/30+91+457+222=0.15125.
The prune trees has the higher training error rates.
...
Your code:
```{r,echo=TRUE}
test_prune_oj <-  predict(prune_oj,newdata = test_oj,type='class')
table(test_prune_oj,test_oj$Purchase)
```

Your answer:

~~~
the test error rate of prune trees is 30+19/30+19+136+85=0.1815.
Unpruned trees has higher test error rate.
